...
namespace LL2UTM{
	GeoConverter geoConverterWGS84;

	void GeoConverter::selectEllipseModel(EllipseModel model){
		ellipse_model = model;
		double f = 1.0 / getEllipseModel().f_inv;
	...
	/*
	Ellipse array has been moved inside of this method, because when it is in global
	the initialization order made a problem when an instance of GeoConverter is also in global.
	*/
	const Ellipse& GeoConverter::getEllipseModel() const{
		const static Ellipse ellipse[3] = {
...
#pragma region Saskatchewan_Univ_Version
		...
		const Ellipse& ellipse = getEllipseModel();
		...
		double Rm = ellipse.a * (1.0 - e2) / pow(1 - e2*sin_lat*sin_lat, 3.0 / 2.0);
		double Rn = ellipse.a / sqrt(1 - e2*sin_lat*sin_lat);
		double T = tan(lat)*tan(lat);
		double C = he2*cos_lat*cos_lat;
		double A = (lon - ref_lon) * cos_lat;
		double AA = A*A;
		double AAA = AA*A;
		double M = ellipse.a * ((1 - e2 / 4 - 3 * e4 / 64 - 5 * e6 / 256)*lat - (3.0 / 8 * e2 + 3.0 / 32 * e4 + 45.0 / 1024 * e6)*sin(2 * lat)
			+ (15.0 / 256 * e4 + 45.0 / 1024 * e6)*sin(4 * lat) - (35.0 / 3072 * e6)*sin(6 * lat));
		//sprintf(buffer, "%.6lf %.6lf %.6lf %.6lf %.6lf %.6lf %.6lf\n", Rm, Rn, AAA, M, e2, e6, he2);
		//Serial.print(buffer);
